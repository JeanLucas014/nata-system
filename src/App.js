...
function Sidebar({page, setPage, profile, collapsed, setCollapsed}) {
  ...
  return (
    <div style={{width:collapsed?72:260,minHeight:'100vh',background:'#fff',borderRight:`1px solid ${G[100]}`,display:'flex',flexDirection:'column',transition:'width .25s',overflow:'hidden',flexShrink:0,position:'relative'}}>
      ...
      <button onClick={()=>setCollapsed(!collapsed)}
        style={{position:'absolute',top:76,right:-14,width:28,height:28,borderRadius:'50%',background:'#fff',border:`1px solid ${G[200]}`,cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:G[400],transform:collapsed?'rotate(0)':'rotate(180deg)',transition:'transform .25s',boxShadow:'0 1px 3px rgba(0,0,0,.08)',zIndex:10}}>
